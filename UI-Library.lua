...
local redzlib = {
	Themes = { -- Themes: Darker, Dark, Purple
		Darker = {
...
local SetProps, SetChildren, InsertTheme, Create do
	...
	SetChildren = function(Instance, Children)
        if type(Children) == "table" and typeof(Instance) == "Instance" then
	...
	SetProps = function(instance, props)
        ...
        if type(instance) ~= "userdata" or not instance.IsA then
            return instance
        end
        ...
        for prop, value in pairs(props) do
            if type(prop) == "string" and instance[prop] ~= nil then
...
local function CreateTween(Configs)
    ...
    if typeof(Instance) ~= "Instance" or type(Prop) ~= "string" or NewVal == nil then
        return
    end
    ...
    local success, tweenInfo = pcall(function()
        return TweenInfo.new(Time, Enum.EasingStyle.Quint)
    end)
    ...
    local success, tween = pcall(function()
        return TweenService:Create(Instance, tweenInfo, {[Prop] = NewVal})
    end)
...
local function VerifyTheme(Theme)
	...
	for name, _ in pairs(redzlib.Themes) do
		if type(name) == "string" and name == Theme then
			return true
		end
	end
...
local function SaveJson(FileName, save)
	if writefile and type(save) == "table" then
		local json = HttpService:JSONEncode(save)
		local success, result = pcall(function()
		    writefile(FileName, json)
		end)
		if not success then
		    warn("Redzlib: Failed to create json " .. fileName .. " content" .. save)
		end
	end
end
...
local function GetColor(Instance)
	if typeof(Instance) ~= "Instance" or not Instance.IsA then
		return ""
	end
	
	if Instance:IsA("Frame") or Instance:IsA("TextBox") or Instance:IsA("TextButton") then
...
function redzlib:MakeWindow(Configs)
	...
	function Window:Dialog(Configs)
        ...
        for _, Button in pairs(DOptions) do
            if Button and typeof(Dialog) == "Instance" and typeof(Dialog.Button) == "function" then
                local success, result = pcall(function()
                    return Dialog:Button(Button)
                end)
                if not success then
                    warn("Redzlib: Failed to create button in dialog", result)
                end
            end
        end
		return Dialog
	...
	function Window:MakeTab(paste, Configs)
		...
		local function Tabs()
			...
			for _, Tab in pairs(redzlib.Tabs) do
                if Tab and Tab.Cont ~= Container then
                    if Tab.func and typeof(Tab.func.Disable) == "function" then
                        local success, result = pcall(function()
                            Tab.func:Disable()
                        end)
                        if not success then
                            warn("Redzlib: Failed to disable tab:", result)
                        end
                    end
                end
            end
				...
				AddNewOptions = function(List, Clear)
                    if Clear then
                        for _, Option in pairs(Options) do
                            if Option and typeof(Option.Destroy) == "function" then
                                local success = pcall(function()
                                    Option:Destroy()
                                end)
                                if not success then
                                    warn("Redzlib: Failed to remove an option")
                                end
                            end
                        end
                        Options = {}
                    ...
                    if type(List) == "table" then
                        for _, OptionData in pairs(List) do
                            if OptionData then
                                local success, newOption = pcall(AddOption, OptionData)
                                if not success then
                                    warn("Redzlib: Failed to add option", OptionData)
                                end
                            end
                        end
                    end
