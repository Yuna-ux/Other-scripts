...
local redzlib = {
	Themes = {
		Darker = { -- Themes: Darker, Dark, Purple.
...
function redzlib:MakeWindow(Configs)
	...
	function Window:MakeTab(paste, Configs)
		...
		function Tab:AddSlider(Configs)
            ...
            local function UpdateLabel(NewValue, SkipCallback)
            ...
            local function UpdateValues(SkipCallback)
                Indicator.Size = UDim2.new(SliderIcon.Position.X.Scale, 0, 1, 0)
                local SliderPos = SliderIcon.Position.X.Scale
                local NewValue = math.floor(((SliderPos * Max) / Max) * (Max - Min) + Min)
                UpdateLabel(NewValue, SkipCallback)
            end
            ...
            function SetSlider(NewValue, SkipCallback)
                if type(NewValue) ~= "number" then return end
                
                local actualMin, actualMax = Min * Increase, Max * Increase
                local clampedValue = math.clamp(NewValue, actualMin, actualMax)
                
                local SliderPos = (clampedValue - actualMin) / (actualMax - actualMin)
                
                if not SkipCallback then
                    SetFlag(Flag, clampedValue)
                end
                
                CreateTween({ SliderIcon, "Position", UDim2.fromScale(math.clamp(SliderPos, 0, 1), 0.5), 0.3, true })
                
                local Number = math.floor(clampedValue * 100) / 100
                Default, LabelVal.Text = Number, tostring(Number)
            end
            
            SetSlider(Default, true)
            UpdateValues(true)
            
            SliderIcon:GetPropertyChangedSignal("Position"):Connect(function()
                UpdateValues(not isInitializing)
            end)
